...
import { useMemberProgress, useLogMemberProgress } from '@/hooks/useMemberPortal';
import { Card, CardHeader, Badge, Button, Modal, Input, Textarea, SkeletonTable, EmptyState } from '@/components/ui';
import {
  TrendingUp,
  Activity,
  Plus,
...
export default function MyProgressPage() {
  ...
  const logProgressMutation = useLogMemberProgress();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    logProgressMutation.mutate(formData, {
      onSuccess: () => {
  ...
  const fatDiff = calculateDiff(latestLog?.bodyFat, previousLog?.bodyFat);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track your weight, body fat percentage, measurements, and fitness evolution over time.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="!p-4 bg-gradient-to-br from-white to-slate-50 border-slate-200/80 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Body Fat %</p>
              <p className="text-2xl font-extrabold text-slate-900 mt-0.5">
                {latestLog && latestLog.bodyFat ? `${latestLog.bodyFat}%` : '—'}
              </p>
              {fatDiff !== null && (
                <div className="flex items-center gap-1 mt-1">
                  {Number(fatDiff) < 0 ? (
                  ...
                  <span className={`text-xs font-semibold ${Number(fatDiff) < 0 ? 'text-emerald-600' : Number(fatDiff) > 0 ? 'text-amber-600' : 'text-slate-500'}`}>
                    {Math.abs(Number(fatDiff))}% from last log
                  </span>
                </div>
              )}
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              ...
              <p className="text-sm text-slate-500 max-w-sm mx-auto mt-1 mb-6">
                Start logging your weight and measurements to keep track of your fitness gains and milestones.
              </p>
              <Button onClick={() => setIsModalOpen(true)} icon={Plus} size="sm">
                ...
                <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                  {progressLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        {log.bodyFat != null ? (
                          <Badge variant="info" className="font-semibold text-xs px-2.5 py-1">
                            {log.bodyFat}%
                          </Badge>
                        ) : (
                          <span className="text-slate-400">—</span>
      ...
      <Modal
        ...
        title="Log Fitness & Body Metrics"
        description="Record your current weight, body fat, and body measurements to track your progress."
        footer={
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Textarea
            label="Notes or Reflections (Optional)"
            placeholder="Add notes about your workout consistency, diet changes, or how you feel..."
