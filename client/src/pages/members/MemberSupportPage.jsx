...
export default function MemberSupportPage() {
  ...
  const handleTrainerSubmit = (e) => {
    ...
    applyTrainerMutation.mutate(
      {
        experienceYears: parseInt(trainerForm.experienceYears) || 1,
      ...
      {
        onSuccess: () => setTrainerModal(false),
      }
    );
  };
    ...
    <div className="space-y-6 animate-fade-in">
                ...
                <tbody className="divide-y divide-slate-100">
                  {applications.map((app) => (
                    <tr key={app.id} className="hover:bg-slate-50 transition-colors text-xs">
                      ...
                      <td className="py-3 px-4 text-slate-600">{app.experienceYears} Years</td>
      ...
      {trainerModal && (
        <Modal open={trainerModal} onClose={() => setTrainerModal(false)} title="Apply to Become a Trainer">
          <form onSubmit={handleTrainerSubmit} className="space-y-4 text-sm">
            <div>
